package org.chronopolis.rest.api;

/**
 * @author shake
 */
public class Paths {

    protected static final String BAG_ROOT = "api/bags";
    protected static final String TOKEN_ROOT = "api/bags/{id}";
    protected static final String REPAIR_ROOT = "api/bags/{id}";
    protected static final String STORAGE_ROOT = "api/storage";
    protected static final String STAGING_ROOT = "api/bags/{id}";
    protected static final String REPLICATION_ROOT = "api/replications";

}
